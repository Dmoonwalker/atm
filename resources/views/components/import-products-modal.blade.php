...
<div x-data="{ 
    ...
    x-show="show"
    x-on:open-modal.window="show = true; success = false; error = null;"
    ...
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            <form @submit.prevent="
                loading = true;
                error = null;
                fetch('{{ route('profile.import-products') }}', {
                    method: 'POST',
                    headers: {
                    ...
                    <div class="sm:flex sm:items-start">
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            <h3 class="text-lg font-semibold text-gray-900">
                                Import Products to Shop
                            </h3>

                            <!-- Success Message -->
                            <div x-show="success"
                                x-transition
                                class="mt-4 p-4 bg-green-50 rounded-md">
                                <div class="flex">
                                    ...
                                    <div class="ml-3">
                                        <p class="text-sm font-medium text-green-800">
                                            Successfully imported <span x-text="importedCount"></span> products
                                            <span x-show="errorCount > 0" x-text="' (with ' + errorCount + ' errors)'"></span>
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div x-show="loading"
                                ...
                                <div class="flex items-center justify-center">
                                    <svg class="animate-spin h-5 w-5 text-[#FFC403]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                    </svg>
                                    <span class="ml-2 text-sm text-gray-600">Importing products...</span>
                                </div>
                            </div>

                            <div class="mt-4" x-show="!loading && !success">
                                <label for="shop_id" class="block text-sm font-medium text-gray-700">Select Shop</label>
                                <select x-ref="shopSelect" name="shop_id" id="shop_id" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:border-[#FFC403] focus:ring-[#FFC403] rounded-md shadow-sm">
                ...
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    ...
                    <button type="button"
                        ...
                        <span x-show="!success">Cancel</span>
                        <span x-show="success">Close</span>
                    </button>
                </div>
            </form>
        </div>
